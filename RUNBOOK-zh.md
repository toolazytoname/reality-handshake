...
**gfwlist 列表明明在、却所有被墙域名都解析出污染 IP**（现代 OpenWrt）：八成是 `confdir` 丢了——`/etc/dnsmasq.d/` 整个目录根本没被加载。一行验证：

```bash
grep conf-dir /var/etc/dnsmasq.conf.*   # 必须是 /etc/dnsmasq.d，如果是 /tmp/dnsmasq.d 就是它
```

修复：`uci set dhcp.@dnsmasq[0].confdir='/etc/dnsmasq.d' && uci commit dhcp && /etc/init.d/dnsmasq restart`。

**重启后配置改动消失 / 服务"明明 enable 了却不自启" / SSH 每次开机都提示 host key 变了**：这三个症状一起出现 = 闪存 overlay（jffs2）腐坏，不是闹鬼。确认：

```bash
dmesg | grep jffs2   # 健康 = 1~10 秒挂完；腐坏 = 60 秒以上 + "Node header CRC failed"
```

根治流程（**先备份再动手**，overlay 上有全部配置、SSH 密钥、装的软件包）：

```bash
# 1. 备份（在电脑上跑）
ssh <路由器> "tar -C /overlay -cf - upper" > router-backup.tar

# 2. 出厂重置（用户确认后！会清空 overlay 上的一切）
ssh <路由器> "firstboot -y && reboot"

# 3. 出厂状态 SSH 免密可进（空密码）
sshpass -p '' ssh -o PreferredAuthentications=password root@192.168.1.1

# 4. 恢复（scp 必须加 -O，dropbear 没有 sftp；恢复后必须重启才能看到文件——overlayfs 不刷新）
scp -O router-backup.tar root@192.168.1.1:/tmp/
ssh <路由器> "tar -C /overlay -xf /tmp/router-backup.tar && rm /tmp/router-backup.tar && reboot"
```
